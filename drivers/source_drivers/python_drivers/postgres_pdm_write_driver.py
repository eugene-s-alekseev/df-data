...
from src.data_model import AppInfo, DraftInfo, DriverInfo
from src.driver_construction import WriteDriver


class PostgresPdmWriteDriver(WriteDriver):
    ...
    def write(self, dataset: pd.DataFrame, draft_info: DraftInfo, app_info: AppInfo, **discovery_params) -> None:
        ...
        dataset.to_sql(source_name, target_engine, if_exists="replace", index=False)

    def get_source_name(self, draft_info: DraftInfo, app_info: AppInfo) -> str:
        return "dl." + draft_info.draft_name + "_" + app_info.app_name
